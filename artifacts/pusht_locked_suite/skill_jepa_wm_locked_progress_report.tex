...
{\large Push-T Locked Suite Status in \texttt{jepa-wms}}\\[0.4em]
...
This round completed the first locked-suite deliverable: a cache-consistent, deterministic, 100-episode reevaluation of the current best Push-T checkpoint under the frozen protocol.
The hierarchical and flat planners tied on success rate at \textbf{7\%}, but the hierarchical planner improved mean pose distance from \textbf{355.30} to \textbf{264.43}, reduced average planning latency from \textbf{0.564 s} to \textbf{0.249 s}, and beat flat on \textbf{57\%} of episodes.
...
I reran the current best Push-T debug checkpoint on \textbf{100 evaluation episodes} under the locked online evaluator and compared hierarchical vs.\ flat planning from the same checkpoint.

\begin{center}
\begin{tabular}{lrrrr}
\toprule
\rowcolor{ReportBlue!10}
Method & Success & Pose Dist. & Planning Latency & Notes \\
\midrule
...
\begin{itemize}
\item hierarchical better-than-flat rate by pose distance: \textbf{0.57},
...
\item \texttt{outputs/skill\_jepa/pusht\_locked\_suite/evals/current\_best\_checkpoint\_100ep/pusht\_online\_eval.json}
\item \texttt{outputs/skill\_jepa/pusht\_locked\_suite/evals/current\_best\_checkpoint\_100ep/pusht\_online\_records.csv}
\item \texttt{outputs/skill\_jepa/pusht\_locked\_suite/reports/current\_best\_checkpoint\_comparison.csv}
\end{itemize}
...
The new reevaluation materially weakens the earlier short-run headline.
...
\item hierarchy does \textbf{not} improve success rate over flat on the current best debug checkpoint.
\end{itemize}

So the current evidence supports this narrower claim:

\begin{quote}
Under the locked debug checkpoint, hierarchy improves control quality and planning efficiency, but the stronger claim that hierarchy improves success rate is not supported by the 100-episode reevaluation.
\end{quote}

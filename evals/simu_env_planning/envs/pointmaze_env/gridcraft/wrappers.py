        ...
        for tile_type in self.types:
            ...
            if len(valid_idxs) == 0:
                ...
                if np.any(dists <= 1.0):
                    on_target = 1.0
                eye_data = np.abs(
                    np.array([on_target, np.max(cosines), np.min(cosines), np.max(sines), np.min(sines)])
                )
...
class RandomObsWrapper(GridObsWrapper):
    def __init__(self, env, dO):
        ...
        self.__observation_space = Box(
            np.min(self.obs_matrix), np.max(self.obs_matrix), shape=(self.dO,), dtype=np.float32
        )

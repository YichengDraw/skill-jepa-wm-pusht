...
def check_vertical_wall_intersect(pos1, pos2, wall_x, hole_y, door_space):
    ...
    if check_intersection:
        ...
        if hole_y is None or y < hole_y - door_space or y > hole_y + door_space:  # we're not in the hole
...
def check_horizontal_wall_intersect(pos1, pos2, wall_y, hole_x, door_space):
    check_intersection = (torch.sign(pos1[1] - wall_y) * torch.sign(pos2[1] - wall_y)) <= 0.1
    if check_intersection:
        d = pos2 - pos1
        a = d[1] / d[0]
        b = pos1[1] - a * pos1[0]
        x = (wall_y - b) / a
        if hole_x is None or x < hole_x - door_space or x > hole_x + door_space:  # we're not in the hole
...
def check_wall_intersect(
    ...
    horizontal_intersect = check_horizontal_wall_intersect(pos1, pos2, top_wall, top_hole, door_space)
    if horizontal_intersect is None:
        horizontal_intersect = check_horizontal_wall_intersect(pos1, pos2, bot_wall, bot_hole, door_space)

...
class DotWall(gym.Env):
    ...
    def reset(self, location=None, **kwargs):
        if location is None:  # always None
            location = self.reset_to_state  # self.reset_to_state should not be None
        self.wall_img = self._render_walls(self.wall_x, self.hole_y)
        if location is None:
            # self._generate_start_and_target()
            dot_position = self.generate_random_state()[0]
    ...
    def _generate_start_and_target(self):
        ...
        if self.cross_wall:
            if self.level == "easy":
                ...
                assert (
                    self.wall_config.wall_padding - self.wall_config.wall_width // 2 - self.wall_config.border_wall_loc
            ...
            start_y = torch.tensor(min_y + random.random() * (max_y - min_y), device=self.device)
            target_y = torch.tensor(min_y + random.random() * (max_y - min_y), device=self.device)
            ...
            location = (
                torch.from_numpy(self.rng.random(size=(4,)) * effective_range + self.padding).to(self.device).float()
            )
            if self.level == "easy":
                ...
                dist = (torch.rand(1) * (max_dist_from_start - min_dist_from_start) + min_dist_from_start).to(
                    location.device
                )
                # set new x and y for goal
                location[2] = location[0] + dist * torch.cos(angle)
                location[3] = location[1] + dist * torch.sin(angle)
                location = torch.clamp(location, min=self.padding, max=self.img_size - 1 - self.padding)
    ...
    def _render_dot(self, location):
        ...
        img = torch.exp(-(c - location).norm(dim=-1).pow(2) / (2 * self.dot_std * self.dot_std)) / (
    ...
    def render(self, **kwargs):
        visual_obs = self.channels_to_img(self.wall_img, self.dot_img)
        return visual_obs  # [C, H, W]
